...
class ProviderInfo:
    ...
    def __init__(self, name: str, approve_address: str, swap_address: str | None = None, icon_path: str | None = None):
        self.name = name
...
UNKNOWN_PROVIDER = ProviderInfo(
    ...
    approve_address="",
    swap_address=None,
    icon_path="A:/res/provider-unknown.png",
)


def provider_by_chain_address(chain_id: int, address: str) -> ProviderInfo | None:
    """根据链ID和地址查找provider"""
    print(f"# provider_by_chain_address: {chain_id}, {address}")
...
def _provider_iterator(chain_id: int) -> Iterator[tuple[str, str, str | None]]:
    ...
    # 注释：以下链ID对应的网络在 evm_tokens 文件夹中未找到对应文件，但渠道商支持
