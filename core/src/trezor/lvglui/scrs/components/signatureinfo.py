...
class AddressFormatter:
    ...
    def truncate_address(self, address: str, start_chars: int = 6, end_chars: int = 4) -> str:
...
class DirectionComponent:
    ...
    def __init__(
        ...
        self.container = ContainerFlexCol(
            parent, None, padding_row=0, no_align=True
        )
        ...
        if to_address:
            DisplayItem(self.container, _(i18n_keys.LIST_KEY__TO__COLON), 
                       self.address_helper.format_address(to_address))
        
        if from_address:
            DisplayItem(self.container, _(i18n_keys.LIST_KEY__FROM__COLON), 
                       self.address_helper.format_address(from_address))
        
        if signer:
            DisplayItem(self.container, _(i18n_keys.LIST_KEY__SIGNER__COLON), 
                       self.address_helper.format_address(signer))
        
        if fee_payer:
            DisplayItem(self.container, _(i18n_keys.LIST_KEY__FEE_PAYER__COLON), 
                       self.address_helper.format_address(fee_payer))
        
        if voter:
            DisplayItem(self.container, _(i18n_keys.LIST_KEY__VOTER__COLON), 
                       self.address_helper.format_address(voter))
        
        if delegator:
            DisplayItem(self.container, _(i18n_keys.LIST_KEY__DELEGATOR__COLON), 
                       self.address_helper.format_address(delegator))
        
        if validator:
            DisplayItem(self.container, _(i18n_keys.LIST_KEY__VALIDATOR__COLON), 
...
class AmountComponent:
    ...
    def __init__(
        ...
        if token_amount:
            DisplayItem(self.container, _(i18n_keys.LIST_KEY__TOKEN_AMOUNT__COLON), token_amount)
...
class FeeComponent:
    ...
    def __init__(
        ...
        if maximum_fee:
            DisplayItem(self.container, _(i18n_keys.LIST_KEY__MAXIMUM_FEE__COLON), maximum_fee)
        ...
        if gas_limit:
            DisplayItem(self.container, _(i18n_keys.LIST_KEY__GAS_LIMIT__COLON), gas_limit)
        
        if gas_fee_cap:
            DisplayItem(self.container, _(i18n_keys.LIST_KEY__GAS_FEE_CAP__COLON), gas_fee_cap)
        
        if gas_premium:
            DisplayItem(self.container, _(i18n_keys.LIST_KEY__GAS_PREMIUM__COLON), gas_premium)
        
        if tip:
            DisplayItem(self.container, _(i18n_keys.LIST_KEY__TIP__COLON), tip)
        
        if total_amount:
            DisplayItem(self.container, _(i18n_keys.LIST_KEY__TOTAL_AMOUNT__COLON), total_amount)
...
class MoreInfoComponent:
    ...
    def __init__(
        ...
        if chain_id is not None:
            DisplayItem(self.container, _(i18n_keys.LIST_KEY__CHAIN_ID__COLON), str(chain_id))
        
        if chain_name:
            DisplayItem(self.container, _(i18n_keys.LIST_KEY__CHAIN_NAME__COLON), chain_name)
        
        if contract_address:
            DisplayItem(self.container, _(i18n_keys.LIST_KEY__CONTRACT_ADDRESS__COLON), contract_address)
        
        if token_id is not None:
            DisplayItem(self.container, _(i18n_keys.LIST_KEY__TOKEN_ID__COLON), str(token_id))
        
        if network_magic is not None:
            DisplayItem(self.container, _(i18n_keys.GLOBAL_TARGET_NETWORK), str(network_magic))
        
        if asset_id:
            DisplayItem(self.container, _(i18n_keys.LIST_KEY__ASSET_ID__COLON), asset_id)
        ...
        if destination_tag is not None:
            DisplayItem(self.container, _(i18n_keys.LIST_KEY__DESTINATION_TAG__COLON), str(destination_tag))
...
class DataComponent:
    ...
    def _add_view_button(self):
        ...
        self.view_button = NormalButton(
            self.card_item.content, 
            _(i18n_keys.BUTTON__VIEW_DATA)
        )
        ...
        self.view_button.add_event_cb(self._on_view_data, 13, None)  # lv.EVENT.CLICKED = 13
